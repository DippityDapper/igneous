...
- New test `[engine][init]` simulates init failure (mock or inject a failing init step) and verifies `Run` returns non-zero without crashing.
- Successful init still passes existing tests unchanged.
...
- Extend `[engine][vec2]` with float dot/magnitude cases that fail today (e.g. `{0.1f, 0.2f}` magnitude, non-axis-aligned dot products).
- Add `[engine][vec3]` tests for float `Dot`, `Magnitude`, and `Normalized` on fractional values.
- All vec tests pass.
...
- New test `[resources][audiostream]` constructs an `AudioStream` and calls `Play` without prior `LoadSound`; must not crash.
- Test passes when all tracks are busy (steals oldest track) and when the mixer is initialized.
...
- Compile a minimal TU that includes only `IdentityProvider.hpp` (add a compile test or a `[networking][headers]` test file).
- Build succeeds with `-Werror` if the project enables it for tests.
...
- New test `[networking][serializer][header]` builds a packet with `Serializer`, reads it back through `PacketRouter::DispatchMessage`, and asserts handler payload matches.
- Test fails if the header size constant is wrong.
...
- Return type is `uint16_t`; out-of-range values are rejected or clamped with a logged warning.
- New test `[engine][cfgparser]` covers `WriteUInt16` / `GetUInt16` round trip and an out-of-range value.
...
- Docs state that messages are **not** portable across endianness without an explicit conversion layer.
- Round-trip test passes on CI (little-endian Linux).
...
- New test `[engine][window]` (or extend `[rendering][window]`) verifies custom dimensions/title after init via the new API.
- Existing examples compile without changes (defaults unchanged).
...
- New test `[rendering][window][resize]` pumps a synthetic `SDL_EVENT_WINDOW_RESIZED` through `Engine::HandleEvents` (or `Window` API) and asserts viewport and resize flag stay in sync.
- `Input::WasWindowResized()` (or replacement) reflects the same event once.
...
- After `Engine::Clean()` or test reset, `SceneManager::GetSceneRoot()` is null, input layers are empty except `_default`, and `Camera::main` is null.
- `[scenes][scenemanager]` and `[input]` tests use reset and pass in isolation and in full suite order.
...
- Move to `Engine::ThreadSafeQueue` (keep a deprecated alias if needed for one release).
- `[engine][threadsafqueue]` passes; no global-namespace `ThreadSafeQueue` references remain in `include/` or `src/`.
...
- `[scenes][sceneroot]` still passes.
- Grep shows no `reinterpret_cast` in `SceneRoot.hpp`.
...
- `[scenes][scene]` and `[scenes][sceneroot]` pass.
- Example scenes compile unchanged.
...
- `[engine][event]` passes.
- If `Ret` is kept, add a test proving return values are used (or document that only `void` is supported and `static_assert` otherwise).
...
### 6.1 Replace `RemoteNetwork` references

`RemoteNetwork` was removed; transport is [ENetNetwork](classes/ENetNetwork.md). Stale links remain in [code-reference.md](code-reference.md), [NetworkInterface.md](classes/NetworkInterface.md), and [tech-debt.md](tech-debt.md).

**Acceptance criteria**

- `rg RemoteNetwork docs/` returns no matches (except historical changelog notes if any).
- [NetworkInterface.md](classes/NetworkInterface.md) lists `LocalNetwork`, `ENetNetwork`, `SteamNetwork`.
...
- Every public header under `include/igneous/networking/` has a matching row in `architecture.md` with a doc link.
- Links resolve (file exists).
...
- Spot-check script or CI step: every `include/igneous/**/*.hpp` either has a `// Doc:` line or is listed in an explicit exceptions table in [code-reference.md](code-reference.md).
- `ResourceManager` and `AudioStream` headers have `@brief` summaries matching their class docs.
...
- Each new tag appears in [Tests](tests.md).
- `./build/tests/igneous_tests` passes locally and in CI.
- Coverage excludes optional Steam builds unless `IGNEOUS_STEAM=ON` (document skipped tags in Tests doc).
...
- Test `[resources][resourcemanager]` registers multiple sprites and asserts IDs are unique and strictly increasing in a single run.
- Unregister + re-register gets a new ID (document behavior).
...
- No unused `PacketType` values remain without a comment explaining wire compatibility.
- `[networking][packetrouter]` still passes.
...
- Test `[networking][enet]` creates a local-only server/client session without UPnP threads (existing `localOnly` path).
- New test confirms UPnP thread is not started when disabled.
...
- Document in [SteamNetwork.md](classes/SteamNetwork.md) and build docs.
- Test `[networking][steam][stub]` (runs only without Steam): calling `Connect()` does not crash and leaves `Connected() == false`.
...
- `[engine][perlin]` passes unchanged.
- A clean rebuild touching only a non-Perlin TU does not recompile Perlin PNG code (verify with `ninja -d explain` or equivalent).
